...
**Первый полноценный сезон:** взнос 0.01 ETH, запись 3 дня, торговля 14 дней. Дата старта ещё не объявлена — до объявления `FIRST_SEASON.opensAt = null` в `packages/shared/src/launch.ts` (там же объявленные параметры и `announcedSchedule`), интерфейс показывает «дата скоро». Когда дата будет:

1. новый чистый деплой (без бета-сезона), первый сезон создаётся сразу с `entryOpensAt` = дата старта — он виден в контракте в фазе `Upcoming`, и таймер может считать до его `entryOpensAt` прямо из сети;
2. дата вписывается в `FIRST_SEASON.opensAt`, новый адрес — в `DEPLOYMENTS[4663].game` (до этого там `null`).
